...
export default function Update ({currentData, mutate, url}){
    const [tanggal, setTanggal] = useState(moment(currentData.tanggal).format('YYYY-MM-DD'));
    ...
    async function handleSubmit(e) {
        ...
        e.preventDefault()
        const res = await fetch (`${process.env.NEXT_PUBLIC_MYSQL_HOST}/api/trackpenerimaan/${currentData.id}`, {
            method : "PATCH", 
            headers : {
                            ...
                            <div className="p-4 overflow-y-auto">
                                <div className="grid gap-4 mb-4 md:grid-cols-2">
                                    ...
                                    <div>
                                        <label 
                                            htmlFor="deskripsi" 
                                            className="block mb-2 text-sm font-medium text-gray-900 dark:text-white text-start">Deskripsi</label>
                                        <input 
                                            type="text" 
                                            id="deskripsi" 
                                            className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" 
